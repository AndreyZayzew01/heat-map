import { heatmapBlocks, type HeatmapAsset, type HeatmapBlock, type HeatmapTribe } from './mockHeatmap.js'
...
function cloneBlocks(blocks: HeatmapBlock[]) {
  return blocks.map((block) => ({
    ...block,
    tribes: block.tribes.map((tribe) => cloneTribe(tribe)) as HeatmapBlock['tribes'],
  }))
}
...
export function getHeatmapPreset(presetId: string | undefined) {
  ...
  const heatmapPresetMap = new Map(getHeatmapPresets().map((preset) => [preset.id, preset] as const))
