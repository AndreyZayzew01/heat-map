...
import { createAssets, heatmapBlocks, type HeatmapBlock } from '../src/data/mockHeatmap.js'

function createTestBlock(): HeatmapBlock {
  return {
    id: 'block-test',
    tribes: [
      {
...
function readWorktreeFile(...segments: string[]) {
...
test('keeps treemap tiles visually stable on hover', () => {
  const options = createHeatmapTreemapOptions(heatmapBlocks[0], 0)
  const treemapOptions = options.plotOptions?.treemap
  const states = treemapOptions?.states as
    | {
...
test('uses requested two-color base background for leaf tiles', () => {
  ...
  for (const point of seriesData) {
    ...
    const gradient = point.color as { stops?: [number, string][] }
    const stops = gradient.stops?.map((stop: [number, string]) => stop[1])
