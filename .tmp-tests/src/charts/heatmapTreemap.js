import { getHeatmapTone, normalizeHeatmapAssetPercents, } from '../data/mockHeatmap.js';
const TRIBE_LABELS = [
    '\u0422\u0440\u0430\u0439\u0431 \u0412\u0435\u0440\u0445\u043D\u0438\u0439',
    '\u0422\u0440\u0430\u0439\u0431 \u041B\u0435\u0432\u044B\u0439',
    '\u0422\u0440\u0430\u0439\u0431 \u041F\u0440\u0430\u0432\u044B\u0439',
...
function createToneGradient(topColor, bottomColor) {
    return {
...
function mergeTinyAssets(tribeId, tribeLabel, assets, tribeWeight) {
    const sortedBySize = [...assets].sort((left, right) => left.normalizedPercent - right.normalizedPercent);
    const tinyAssetIds = new Set();
    ...
    const baseNodes = stableAssets.map((asset, assetIndex) => createLeafNode(tribeId, tribeLabel, asset, getAssetLabel(assetIndex), tribeWeight));
...
function getTribeLabelMarkup(label) {
    return `<span class="heatmap-tribe-label">${label}</span>`;
}
export function getAssetLabelMarkup(point, tileWidth = Number.POSITIVE_INFINITY) {
...
function ensureLeafVisualDefs(chart) {
    const chartWithDefs = chart;
    ...
    const ensureStop = (gradient, offset, color) => {
    ...
    ;
    ['critical', 'warning', 'healthy'].forEach((tone) => {
...
function applyLeafSvgStyles(chart) {
...
export function createHeatmapTreemapSeriesData(block) {
...
export function createHeatmapTreemapOptions(block, blockIndex) {
    return {
        chart: {
            type: 'treemap',
                ...
                levels: [
                    ...
                    },
                    {
                        ...
                        dataLabels: {
                            ...
                            formatter: function () {
                                const point = this;
                                return getAssetLabelMarkup(point.point?.options ?? point.options ?? { id: '', name: '' }, point.point?.shapeArgs?.width ?? point.shapeArgs?.width);
                            },
                            style: {
                                color: CHART_TEXT_COLOR,
                                fontSize: '12px',
