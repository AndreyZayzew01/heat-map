...
function collectButtons(node, result = []) {
    ...
    collectButtons(node.props.children, result);
    return result;
}
function findHeatMapPanel(node) {
    if (Array.isArray(node)) {
        for (const child of node) {
    ...
    return findHeatMapPanel(node.props.children);
}
function getButtonText(children) {
...
test('renders three preset buttons and current mode label', () => {
    const markup = renderToStaticMarkup(_jsx(App, {}));
