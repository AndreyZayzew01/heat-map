...
function cloneBlocks(blocks) {
    return blocks.map((block) => ({
        ...block,
        tribes: block.tribes.map((tribe) => cloneTribe(tribe)),
    }));
}
function normalizePercents(values, targetTotal) {
    ...
    if (rawTotal <= 0) {
        ...
        return values.map((_, index) => (evenShare + (index < remainder ? 1 : 0)) / ROUNDING_FACTOR);
    }
    const scaled = values.map((value) => (Math.max(value, 0) / rawTotal) * totalUnits);
    ...
    for (let index = 0; index < indicesByFraction.length && remainder > 0; index += 1) {
        units[indicesByFraction[index].index] += 1;
        remainder -= 1;
    }
    return units.map((value) => value / ROUNDING_FACTOR);
}
function getWeight(weights, index) {
    return weights[Math.min(index, weights.length - 1)] ?? 1;
}
function getElevatedValue(value, index) {
    const reduction = index === 0 ? 3 : index < 3 ? 2 : 1;
    return clamp(value - reduction, 1, 10);
}
function getCriticalValue(value, index) {
    const reduction = index === 0 ? 6 : index < 3 ? 4 : 3;
    return clamp(value - reduction, 1, 10);
}
function transformAssets(assets, weights, getScenarioValue) {
    const targetTotal = Math.min(sumPercents(assets), 100);
    const weightedPercents = assets.map((asset, index) => asset.percent * getWeight(weights, index));
...
function transformBlocks(blocks, weights, getScenarioValue) {
    return blocks.map((block) => ({
        ...block,
...
const heatmapPresetDefinitions = [
    {
        id: 'default',
        label: '\u0411\u0430\u0437\u043E\u0432\u044B\u0439',
        description: '\u0411\u0430\u0437\u043E\u0432\u0430\u044F \u043A\u0430\u0440\u0442\u0438\u043D\u0430 \u043F\u043E \u043C\u0435\u0442\u0440\u0438\u043A\u0430\u043C \u0438 \u0440\u0430\u0441\u043F\u0440\u0435\u0434\u0435\u043B\u0435\u043D\u0438\u044E \u0434\u043E\u043B\u0435\u0439.',
        blocks: () => cloneBlocks(heatmapBlocks),
    },
    {
        id: 'elevated',
        label: '\u041F\u043E\u0432\u044B\u0448\u0435\u043D\u043D\u044B\u0439 \u0440\u0438\u0441\u043A',
        description: '\u0423\u043C\u0435\u0440\u0435\u043D\u043D\u043E \u0443\u0445\u0443\u0434\u0448\u0435\u043D\u043D\u044B\u0439 \u0441\u0446\u0435\u043D\u0430\u0440\u0438\u0439 \u0441 \u0440\u043E\u0441\u0442\u043E\u043C warning \u0438 critical \u0437\u043E\u043D.',
        blocks: () => transformBlocks(cloneBlocks(heatmapBlocks), ELEVATED_WEIGHTS, getElevatedValue),
    },
    {
        id: 'critical',
        label: '\u041A\u0440\u0438\u0437\u0438\u0441\u043D\u044B\u0439',
        description: '\u041A\u0440\u0438\u0437\u0438\u0441\u043D\u044B\u0439 \u0441\u0446\u0435\u043D\u0430\u0440\u0438\u0439 \u0441 \u044F\u0432\u043D\u044B\u043C \u0441\u043C\u0435\u0449\u0435\u043D\u0438\u0435\u043C \u0432 critical-\u043C\u0435\u0442\u0440\u0438\u043A\u0438.',
        blocks: () => transformBlocks(cloneBlocks(heatmapBlocks), CRITICAL_WEIGHTS, getCriticalValue),
    },
];
...
export function getHeatmapPreset(presetId) {
    const normalizedPresetId = presetId ?? DEFAULT_PRESET_ID;
    const heatmapPresetMap = new Map(getHeatmapPresets().map((preset) => [preset.id, preset]));
    const preset = heatmapPresetMap.get(normalizedPresetId);
    if (preset) {
        return preset;
    }
    console.warn(`Unknown heatmap preset id "${normalizedPresetId}", falling back to default.`);
